...
class ScormXBlockTests(unittest.TestCase):
    ...
    def test_collect_course_scorm_sha1s_override_replaces_stale_draft(self):
        # The draft branch returns a stale sha1 for the block currently being
        # edited because field changes aren't persisted to the modulestore
        # until studio_submit returns. The override must substitute the
        # in-memory sha1 so the previous draft zip falls out of the
        # referenced set and gets reclaimed.
        block_x_published = mock.Mock()
        block_x_published.scope_ids.usage_id = "usage_X"
        block_x_published.package_meta = {"sha1": "published_x"}

        block_x_draft_stale = mock.Mock()
        block_x_draft_stale.scope_ids.usage_id = "usage_X"
        block_x_draft_stale.package_meta = {"sha1": "stale_draft_x"}

        block_y_draft = mock.Mock()
        block_y_draft.scope_ids.usage_id = "usage_Y"
        block_y_draft.package_meta = {"sha1": "draft_y"}

        store = mock.Mock()

        def fake_get_items(_course_key, qualifiers, revision):
            if revision == "published_only":
                return [block_x_published]
            if revision == "draft_only":
                return [block_x_draft_stale, block_y_draft]
            return []

        store.get_items = mock.Mock(side_effect=fake_get_items)

        ModuleStoreEnum = mock.Mock()
        ModuleStoreEnum.RevisionOption.published_only = "published_only"
        ModuleStoreEnum.RevisionOption.draft_only = "draft_only"

        result = ScormXBlock._collect_course_scorm_sha1s(
            store,
            "course-key",
            ModuleStoreEnum,
            override_usage_id="usage_X",
            override_draft_sha1="fresh_draft_x",
        )

        self.assertIn("published_x", result)
        self.assertIn("fresh_draft_x", result)
        self.assertIn("draft_y", result)
        self.assertNotIn("stale_draft_x", result)

    def test_collect_course_scorm_sha1s_no_override_keeps_modulestore_value(self):
        block_x_draft = mock.Mock()
        block_x_draft.scope_ids.usage_id = "usage_X"
        block_x_draft.package_meta = {"sha1": "draft_x"}

        store = mock.Mock()
        store.get_items = mock.Mock(return_value=[block_x_draft])
        ...
        result = ScormXBlock._collect_course_scorm_sha1s(
            store, "course-key", ModuleStoreEnum
        )

        self.assertEqual(result, {"draft_x"})
